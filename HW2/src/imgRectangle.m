function imgOut = imgRectangle(image)
	imgOut = image;

	% get initial sub image
	sub_region = getSubRegion(imgOut);
...
function sub_region = getSubRegion(image)
	...
	for c = 1:col
		...
		tmp = c;
		while(tmp <= col && image(1, tmp, 1) == 0 && image(1, tmp, 2) == 0 && image(1, tmp, 3) == 0)
			tmp = tmp + 1;
		end
		len = tmp - c;
	...
	for c = 1:col		
		...
		tmp = c;
		while(tmp <= col && image(row, tmp, 1) == 0 && image(row, tmp, 2) == 0 && image(row, tmp, 3) == 0)
			tmp = tmp + 1;
		end
		len = tmp - c;
	...
	for r = 1:row
		...
		tmp = r;
		while(tmp <= row && image(tmp, 1, 1) == 0 && image(tmp, 1, 2) == 0 && image(tmp, 1, 3) == 0)
			tmp = tmp + 1;
		end
		len = tmp - r;
	...
	for r = 1:row 
		...
		tmp = r;
		while(tmp <= row && image(tmp, col, 1) == 0 && image(tmp, col, 2) == 0 && image(tmp, col, 3) == 0)
			tmp = tmp + 1;
		end
		len = tmp - r;
